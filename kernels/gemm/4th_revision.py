...
# 5-stage buffering is optimal for all K sizes
num_acc_stage = 1
num_ab_stage_optimal = 5  # Optimal for all benchmark sizes
...
def kernel(
    ...
    num_ab_stage: cutlass.Constexpr[int],  # Size-adaptive pipeline depth
):
    """
    ...
    #
    # Prefetch TMA descriptors to hide descriptor fetch latency
    #
    ...
    # Software pipelining with num_ab_stage=3 (triple buffering):
    # 1. Prologue: Issue loads for first num_ab_stage tiles
    # 2. Main loop: Compute tile k while loading tile k+num_ab_stage
    # 3. Epilogue: Drain remaining tiles
    #
    if warp_idx == 0:
        ...
        # Number of kblocks for the inner loop
        num_kblocks = cute.size(tCrA, mode=[2])
        
        #
        # PROLOGUE: Fill the pipeline by issuing first min(num_ab_stage, k_tile_cnt) loads
        # We use range_constexpr for unrolling but guard with if for runtime bounds
        #
        for prologue_k in cutlass.range_constexpr(num_ab_stage):
            if prologue_k < k_tile_cnt:
                # Acquire empty buffer and issue TMA load
                ab_empty = ab_producer.acquire_and_advance()
                cute.copy(
                    tma_atom_a,
                    tAgA[(None, prologue_k)],
                    tAsA[(None, ab_empty.index)],
                    tma_bar_ptr=ab_empty.barrier,
                )
                cute.copy(
                    tma_atom_b,
                    tBgB[(None, prologue_k)],
                    tBsB[(None, ab_empty.index)],
                    tma_bar_ptr=ab_empty.barrier,
                )
                cute.copy(
                    tma_atom_sfa,
                    tAgSFA[(None, prologue_k)],
                    tAsSFA[(None, ab_empty.index)],
                    tma_bar_ptr=ab_empty.barrier,
                )
                cute.copy(
                    tma_atom_sfb,
                    tBgSFB[(None, prologue_k)],
                    tBsSFB[(None, ab_empty.index)],
                    tma_bar_ptr=ab_empty.barrier,
                )
        
        #
        # MAIN LOOP: Process tiles with pipelining
        # With multi-buffering, the next tile's data is already being loaded while we compute
        #
        for k_tile in range(k_tile_cnt):
            # Wait for data of tile k to be ready
            ...
            # Release buffer for reuse - allows producer to load next tile
            ab_full.release()
            
            # Issue load for next tile after releasing current buffer
            # This load happens in background while we process next iteration
...
def my_kernel(
    ...
    kernel(
        ...
        # Size-adaptive pipeline depth
        num_ab_stage,               # Number of pipeline stages (varies by K size)
    ).launch(
        grid=grid,
...
# Global cache for compiled kernels - one per stage configuration
# Key: num_ab_stage, Value: compiled kernel
_compiled_kernel_cache = {}

def get_optimal_stages(k: int) -> int:
    """
    Get optimal pipeline stages.
    
    After optimization, 5 stages is best for all benchmark K sizes.
    The Constexpr parameter approach provides better codegen than global variables.
    """
    return num_ab_stage_optimal  # 5 stages for all

def compile_kernel_variant(num_stages: int):
    """
    Compile a kernel variant with specific number of pipeline stages.
    """
    global _compiled_kernel_cache
    
    if num_stages in _compiled_kernel_cache:
        return _compiled_kernel_cache[num_stages]
    ...
    # Compile the kernel with specific stage count
    compiled = cute.compile(my_kernel, a_ptr, b_ptr, sfa_ptr, sfb_ptr, c_ptr, (0, 0, 0, 0), num_stages)
    _compiled_kernel_cache[num_stages] = compiled
    
    return compiled

def compile_all_kernels():
    """
    Pre-compile all kernel variants to avoid compilation overhead during execution.
    """
    compile_kernel_variant(num_ab_stage_optimal)


def custom_kernel(data: input_t) -> output_t:
    """
    Execute the block-scaled GEMM kernel with size-adaptive optimization.
    
    This is the main entry point called by the evaluation framework.
    It converts PyTorch tensors to CuTe tensors, launches the kernel,
    and returns the result.
    
    Size-adaptive optimization:
    - Large K (>= 8192): 5-stage pipeline for maximum latency hiding
    - Medium K (>= 4096): 4-stage pipeline for balanced performance
    - Small K (< 4096): 2-stage pipeline to reduce overhead
    ...
    # Select optimal pipeline stages based on K size
    num_stages = get_optimal_stages(k)
    
    # Get the compiled kernel variant for this stage count
    compiled_func = compile_kernel_variant(num_stages)
    ...
    # Execute the compiled kernel (num_stages was baked in at compile time)
